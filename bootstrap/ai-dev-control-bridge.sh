...
STATUS: $status
DETAIL: $detail"
  "$GH_BIN" api "repos/$REPO/issues/$INBOX_ISSUE/comments" -f body="$body" >/dev/null 2>&1 || true
}
...
do_doctor() {
  if [ ! -x "$ACTIVE_AI_DEV" ]; then
    echo "ai-dev-not-installed"
    return 4
  fi
...
do_sync_install() {
  ...
  local installer=""
  if [ -x "$FRAMEWORK_DIR/bootstrap/install-ai-dev.sh" ]; then
    installer="$FRAMEWORK_DIR/bootstrap/install-ai-dev.sh"
  elif [ -x "$FRAMEWORK_DIR/tools/codex-runner/install.sh" ]; then
    installer="$FRAMEWORK_DIR/tools/codex-runner/install.sh"
  elif [ -f "$FRAMEWORK_DIR/bootstrap/install-ai-dev.sh" ]; then
    installer="$FRAMEWORK_DIR/bootstrap/install-ai-dev.sh"
...
process_command() {
  local comment_id="$1"
  local body
  body="$("$GH_BIN" api "repos/$REPO/issues/comments/$comment_id" --jq .body 2>/dev/null)" || return 0

  printf '%s\n' "$body" | grep -q '^AI_DEV_COMMAND_BEGIN$' || return 0

  local command_id action commit
  ...
  case "$action" in
    PING)
      output="$(do_ping 2>&1)" || rc=$?
      ;;
    DOCTOR)
      output="$(do_doctor 2>&1)" || rc=$?
      ;;
    DOCTOR_REPAIR)
      output="$(do_doctor_repair 2>&1)" || rc=$?
      ;;
    BRIDGE_UPDATE)
      output="$(do_bridge_update "$commit" 2>&1)" || rc=$?
      ;;
    SYNC_INSTALL)
      output="$(do_sync_install "$commit" 2>&1)" || rc=$?
      ;;
    *)
...
main_once() {
  ...
  local ids
  ids="$("$GH_BIN" api --paginate "repos/$REPO/issues/$INBOX_ISSUE/comments?per_page=100" \
    --jq '.[] | select(.user.login=="tokpmpm" and (.body|contains("AI_DEV_COMMAND_BEGIN"))) | .id' 2>/dev/null)" || {
